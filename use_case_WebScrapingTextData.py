...
def app():
    ...
    if tw_classifier=='Text summarization': 
        ...
        if run_text_OK == True: 
            ...
            extr_methods = st.multiselect("Extractive summarization methods ", ["Luhn",  "Edmunson", "LSA",  "LexRank", "TextRank", "SumBasic","KL-Sum"], ["LSA","SumBasic"], key = st.session_state['key'], on_change=tx_sum_change)
...
#-----------------------------------------------------------------
    if tw_classifier=='Text analysis':
        ...
        if run_text_OK == True: 
            ...
            if stopword_selection=="No stop words (use all words)":
                ...
                if st.checkbox('Show built-in list of stop words', value = False, key = st.session_state['key']): 
                    st.dataframe(pd.DataFrame(word_stopwords,columns=['stop word'])) 
                if st.checkbox('Add additional stop words', value = False, key = st.session_state['key']): 
            ...
            with a5:    
                if draw_WordCloud==True:              
                    ...
                    user_color_name=st.selectbox('Select main color of your WordCloud',color_options.columns, key = st.session_state['key'])
                ...
                if st.session_state['sentiment'] is not None and user_language in ["de", "en"]:
                    ...
                    with sentiment_output:
                        if user_language=="de" and fc.is_localhost()==False: 
                            st.info("German sentiment analysis can be performed on local server only!")        
                        else:
                            st.subheader('Sentiment Analysis')
                            st.write(sa_table.style.set_precision(user_precision))  
                            # Download link
                            st.write("")  
                            output = BytesIO()
                            excel_file = pd.ExcelWriter(output, engine="xlsxwriter")
    ...
    if tw_classifier =='Stock data access via yfinance':  
        ...
        a3, a4 = st.columns(2) 
        with a3:
            first_stock = st.text_input("Enter a stock ticker symbol", "TSLA", key = st.session_state['key'], on_change=in_wid_change)
            symbols_all=list('-')+symbols_all
            second_stock = st.selectbox('You can add an additional stock for comparision...',symbols_all, key = st.session_state['key'], on_change=in_wid_change)
        with a4:
            today = datetime.date.today()
            last_year = today - datetime.timedelta(days=365)
            start_date = st.date_input('Select start date', last_year, key = st.session_state['key'], on_change=in_wid_change)
            end_date = st.date_input('Select end date', today, key = st.session_state['key'], on_change=in_wid_change)
        ...
        with dev_expander_perf:
            ...
            if second_stock !="-":    
                ...
                comparision_check=st.checkbox('Compare '+ first_stock + " & " + second_stock, value = True, key = st.session_state['key'])
